...
const ScrollContextProvider = ({ children }) => {
  ...
  const ResumeRef=useRef(resumelink)
  const handleHomeRef = () => {
  ...
  const handleResumeRef = () => {
    ResumeRef.current?.scrollIntoView({ behavior: "smooth", block: "start", inline: "center" });
  };

  return (
    <ScrollContext.Provider value={{ HomeRef, AboutRef, SkillsRef, ProjectsRef, ContactRef, handleHomeRef, handleAboutRef, handleSkillsRef, handleProjectsRef, handleContactRef,handleResumeRef }}>
      {children}
